...
export default function Admin({ user }){
  ...
  useEffect(() => {
    ...
    const raw = window.sessionStorage.getItem('adminDocFocus')
    if (!raw) return
    window.sessionStorage.removeItem('adminDocFocus')
    try{
      const data = JSON.parse(raw)
      const category = typeof data?.category === 'string' && DOC_CATEGORIES.includes(data.category)
        ? data.category
        : DOC_CATEGORIES[0]
      ...
      setDocSlug(slug)
      setDocsNotice(`Listo para gestionar ${category} de ${slug.toUpperCase()}.`)
      window.setTimeout(() => {
        const el = document.getElementById('admin-docs-card')
  ...
  return (
    <RoleGate user={user} allow={['admin','ri']}>
      <div className="container">
        ...
        <div id="admin-docs-card" className="card" style={{marginTop:12}}>
