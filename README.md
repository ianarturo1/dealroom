...
- **Subir:** UI de `/documents` llama `upload-doc` → commit al repo de documentos.
  - La función rechaza archivos sin bytes, valida la integridad del base64 y confirma que GitHub devuelva un `commitSha` antes de responder `ok:true`.
- **Descargar:** `download-file` entrega el archivo desde GitHub (inline o attachment) para el slug permitido.
  ...
  - Rechaza respuestas vacías (`code: EmptyFile`) o con tamaño distinto al reportado (`code: SizeMismatch`) para que la UI lo muestre claramente.
